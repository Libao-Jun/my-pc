...
- 6. 保护功能：可自定义水印文本、字体和字号、不透明度、旋转角度、单行水印和多行水印（一页两行、一页三行、一页六行、一页八行）、以及文本位置和是否应用于全部页面。
  ...
  - 6.2 给视频添加防伪水印
- 7. 迁移电脑文件或者缓存功能：使用软连接将某盘缓存移到其他盘符，必须以管理员身份运行 CMD才行
  - 7.1 先将原文件夹移动到目标盘
  - 7.2 创建软链接（指向新位置）
  - 7.3 必须自定义文件路径可（必须提供：迁出盘符路径和迁入盘符路径可分别自定义），单个文件执行迁移/也可批量执行文件执行
  - 7.4 可识别哪些文件或者缓存是可以直接迁移（低风险的可直接迁移），哪些文件或缓存是高风险的（不建议迁移）
- 8. 集成markdown功能：集成可打开和编辑 markdown文件的功能，可以正常识别和展示markdown文件支持的内容，类似于 typora 软件
  - 8.1 可打开编辑 markdown 文件，识别 markdown 文件的所有内容
  - 8.2 支持预览（类似：https://cv.ftqq.com/）
  - 8.3 支持导出为 PDF，导出的 PDF 格式内容排版方面必须是高度还原 markdown 原文内容。
